...
<%@page import="com.techgiants.topiefor.status.Status"%>
                ...
                <div id="popup" class="popup">
                    <div class="popup-content">
                        ...
                        <form action="service" method="POST">
                            <centre>
                                <input type ="hidden" id = "pro" name="process"value ="update-order-status">
...
//                                    for (Status state : Status.values()) { state.name()  %>
...
//                                    } %>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><%= df.format(ord.getTotalAmount())%></td>
                            <td><%= dtf.format(ord.getOrderDate())%></td>
                            <td><%= ord.getInvoice().getPayment() != null ? "YES" : "NO"%></td>
                            <td><%= ord.getDelivery().getDeliveryDate() != null ? dtf.format(ord.getDelivery().getDeliveryDate()) : "Not Delivered"%></td>
                            <td>
                                <% 
                                    int state = ord.getStatus().ordinal();
                                    if(state<2){
                                        String statName  = Status.values()[++state].name();
                                %>
                                    <button onclick="openPopup('<%= ord.getOrderId()%>', '<%=statName %>')">Edit</button>
                                <%
                                    }else if(state==2){

                                %>
                                <a  href="service?service=admin&process=delivery-note&order-id=<%=ord.getOrderId() %>"><button>Delivery Note</button></a>
                                <%
                                    }
                                %>
                            </td>
                ...
                <script>
                    function openPopup(orderId, status) {
                        var popup = document.getElementById("popup");
                        popup.classList.add("active");

                        document.getElementById("orderId").value = orderId;
                        
                        var opt = document.getElementById("status");
                        opt.value = status;
